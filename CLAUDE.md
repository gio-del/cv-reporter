...
- `backend/` — Go HTTP API (see `backend/internal/api`) that reads/writes the same Master Data files under `data/` that the skill uses. `backend/cmd/server` is the API's entrypoint; `backend/cmd/cvcheck` is an offline CLI over the Generation quality checks (`generation.CheckSelectionGroundedness`), which the skill shells out to.
