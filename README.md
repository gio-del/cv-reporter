...
Frontend tests are Vitest + Testing Library, rendering the real page inside a real router and faking only HTTP with Mock Service Worker — the API client, its query-string building and its error handling all run for real, and an unhandled request fails the test. They need no backend, no `ANTHROPIC_API_KEY` and no `typst`. See [`docs/adr/0018-frontend-tests-fake-only-the-network.md`](docs/adr/0018-frontend-tests-fake-only-the-network.md) for why that seam, and `frontend/src/pages/GenerationPage.test.tsx` for the example to copy when adding more.
