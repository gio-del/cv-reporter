...
The tailoring "build" is the `tailor-cv` skill (`plugins/cv-reporter-skills/skills/tailor-cv/SKILL.md`, invoked as `/cv-reporter-skills:tailor-cv`) — invoke it with a job description (pasted text or URL), a reference to an already-tracked Application, or nothing (Default Mode). It walks: Selection → Rewrite (plus a Cover Letter draft from `data/cover-letter-snippets/`, or fresh prose, whenever a Job Description is given — not in Default Mode) → Text Review (approval required, CV and Cover Letter approved independently) → Render → Visual Review (approval required), recording the Generation against the tracked Application afterward when that's the input mode used. Along the way it runs the web app's automated quality checks (groundedness before Text Review; page count, ATS-parsability and `lang` before Visual Review) by shelling out to `backend/cmd/cvcheck` through `plugins/cv-reporter-skills/skills/tailor-cv/scripts/quality-check.sh` — the same Go check code the app calls, offline, no running backend needed (ADR-0028). Checks are advisory and never block a checkpoint.
...
- `backend/` — Go HTTP API (see `backend/internal/api`) that reads/writes the same Master Data files under `data/` that the skill uses. `backend/cmd/server` is the API's entrypoint; `backend/cmd/cvcheck` is an offline CLI over the Generation quality checks (`generation.CheckSelectionGroundedness`, `generation.CheckRenderedCV`), which the skill shells out to.
