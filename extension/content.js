...
(function () {
  const { firstNonEmptyText, descriptionMarkdown, longestElement, initCaptureUI } = window.CVReporterCommon;
  ...
  function salaryBadgeText() {
    try {
      const descriptionEl = longestElement(document, DESCRIPTION_SELECTOR);
  ...
  function captureJobPosting() {
    return {
      ...
      company: firstNonEmptyText(document, ['a[href*="/company/"]']),
      location: "",
      url: captureUrl(),
      description: descriptionMarkdown(document, DESCRIPTION_SELECTOR, ["button"]),
      logoUrl: companyLogoUrl(),
      listingSalaryText: salaryBadgeText(),
    };
  }
