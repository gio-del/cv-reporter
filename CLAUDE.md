...
- `backend/` — Go HTTP API (see `backend/internal/api`) that reads/writes the same Master Data files under `data/` that the skill uses. Every write under `data/` goes through `backend/internal/atomicfile`'s `WriteFile` (temp file in the same directory, then rename) so an interrupted write leaves the previous record intact; writes under `output/` stay on plain `os.WriteFile`, since derived artifacts can be regenerated (ADR-0018).
